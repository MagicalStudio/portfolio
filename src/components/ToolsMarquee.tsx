...
function ToolLogo({ tool, isSmall = false }: { tool: { name: string; logo: string }, isSmall?: boolean }) {
  return (
    ...
    >
      <div style={{ width: isSmall ? "28px" : "64px", height: isSmall ? "28px" : "64px", borderRadius: isSmall ? "7px" : "14px", overflow: "hidden", flexShrink: 0, background: "rgba(255,255,255,0.05)" }}>
        <img src={tool.logo} alt={tool.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
      </div>
